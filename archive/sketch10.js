...
function disjointSubsets(x,y,n){
	...
	if(sets.length < n){
		// county+=sets.length;
		// console.log("Here, have some free performance " + county);
		return;
	}
			...
			for(j=x[0];j<x[1];j++){
				for(k=y[0];k<y[1];k++){
					...
					if(!skip){
						for(var a in s_set){
							if(c_grid[j][k][a] == 1){
								pushMessage("Candidate: " + a + " removed from at: " + j + ", " + k);
								delete c_grid[j][k][a];
								c_grid[j][k].len--;
							}							
						}
...
var b1 = gTileSize*9+gOffsetY;
var b2 = gTileSize*9+gOffsetX;
var b3 = gTileSize*9+gOffsetY+1;
var b4 = gTileSize*9+gOffsetX+1;
var b5 = gOffsetY-1;
var b6 = gOffsetY+1;
var b7 = gOffsetX-1
var b8 = gOffsetX+1;
var g1 = gFontOffsetX+gOffsetX;
var g2 = gFontOffsetY+gOffsetY;
var s1 = 9*gTileSize+rOffset;
var s2 = gFontOffsetY+gOffsetY+rFontY;
var s3 = gFontOffsetX+gOffsetX+cFontX;
var s4 = 9*gTileSize+cOffset;
var s5 = sSize+sSpaceX;
var s6 = sSize+sGapX;
var s7 = sSize+sSpaceY;
var s8 = sSize+sGapY;
var m1 = gOffsetX+15;
var m2 = msgFontSize+5;
...
function draw(){
	...
	stroke(borderColour);
	for(i=0;i<=grid_l;i++){
		line((i*gTileSize)+gOffsetX,gOffsetY,(i*gTileSize)+gOffsetX,b1);
		line(gOffsetX,(i*gTileSize)+gOffsetY,b2,(i*gTileSize)+gOffsetY);
	}
	fill(borderColour);
	stroke(borderColour);
	for(i=0;i<4;i++){
		line((i*3*gTileSize-1)+gOffsetX,b5,(i*3*gTileSize)+b7,b3);
		line((i*3*gTileSize+1)+gOffsetX,b5,(i*3*gTileSize)+b8,b3);
		line(b7,(i*3*gTileSize)+b5,b4,(i*3*gTileSize)+b5);
		line(b7,(i*3*gTileSize)+b6,b4,(i*3*gTileSize)+b6);
	}
	
	// Print values
	for(i=0;i<grid_l;i++){
		for(j=0;j<grid_l;j++){
			text(m_grid[i][j], (i*gTileSize)+g1,(j*gTileSize)+g2);
		}
	}
	...
	for(i=0;i<grid_l;i++){
		for(j=0;j<grid_l;j++){
			if(row_set[i][j] != 1){
				text(j+1, s1+(rowCount*rSize), (i*gTileSize)+s2);
				rowCount++;
			}
			if(col_set[i][j] != 1){
				text(j+1, (i*gTileSize)+s3, s4+(colCount*cSize));
				colCount++;
			}
		}
		rowCount = 0;
	...
	for(i=0;i<sub_l;i++){
		for(j=0;j<sub_h;j++){
			for(k=0;k<grid_l;k++){
				if(sub_set[i][j][k] != 1){
					text(	k+1, 
							sTopX+((subCount%sub_l)*s5)+(i*3*s6), 
							sTopY+(Math.floor(subCount/sub_h)*s7)+(j*3*s8));
					subCount++;
				}
			}
			subCount = 0;
	...
	for(i=0;i<len;i++){
		...
		text(msgBuff[i][0],msgOffsetX+(col*340),m1+(row*m2));
		msgBuff[i][1] -= msgFade;
	}
}
